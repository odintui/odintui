...
block_new :: proc() -> Block {
	return Block{borders = BORDERS_ALL, border_set = tui.BORDER_PLAIN}
}
...
block_render :: proc(b: ^Block, area: tui.Rect, buf: ^tui.Buffer) {
	...
	if .Top in b.borders {
		...
		if .Right in b.borders {
			if c := tui.buffer_cell(buf, area.x + area.width - 1, area.y); c != nil {
	...
	if .Bottom in b.borders {
		...
		if .Right in b.borders {
			if c := tui.buffer_cell(buf, area.x + area.width - 1, y); c != nil {
	...
	if len(b.title) > 0 && .Top in b.borders {
		...
		if max_w > 0 {
			tui.buffer_set_string(buf, tx, area.y, b.title, b.title_style, u16(max_w))
		}
	}
}
...
block_widget :: proc(b: ^Block) -> tui.Widget {
	return tui.Widget{data = b, render = proc(data: rawptr, area: tui.Rect, buf: ^tui.Buffer) {
